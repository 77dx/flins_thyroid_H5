<template>
	<view class="video_detail">
					...
					<view v-for="(item,index) in recommendList" :key="index">
						...
						<view class="tab_list_1">
							<view class="tab_list_1_inner">
								...
								<view class="tab_list_botton">
									<view class="watch_people" v-if="item.type == 1">{{item.view_num}}人看过</view>
									<view class="watch_people" v-if="item.type == 2">{{item.view_num}}人看过</view>
									<view class="botton_right">
		...
		<view class="popup_content_video" v-if="is_cover"></view>
		<!-- 视频未达到级别弹框 -->
		...
		<!-- 点击添加评论 -->
		<view class="add_comment_btn" v-if="isComment">
			<input class="add_comment_input" type="text" value="" placeholder="点击这里评论" disabled="" @tap="toComment"/>
		</view>
		<view class="add_comment_btn" v-if="!isComment">
			<input class="add_comment_input" type="text" value="" placeholder="付费购买后才能评论" disabled/>
		</view>
	</view>
</template>
			...
			getVideoDetail(){
				...
				video_info(params).then(res => {
					...
					if(res.data.code == 1){
						...
						if(that.videoDetail.is_view == 0){//可以看
							that.isComment = true
						}else{
